...
def obtainLabels(img):
    ...
    labels_walk = chan_vese(r, mu=0.25, lambda1=1, lambda2=2, tol=1e-3, max_iter=200,
    ...
    labels_walk = morphology.remove_small_objects(labels_walk, 100, connectivity=4)

    # Watershed

    D = ndi.distance_transform_edt(labels_walk)
    localMax = peak_local_max(D, indices=False, min_distance=20, labels=labels_walk)
...
def getGroundTruths(dataset):
    ...
    df.to_csv("ChanVese_Segmentation_report.csv")
