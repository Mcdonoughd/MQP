...
def obtainLabels(img):
    ...
    r = adjust_gamma(r, gamma=0.8)


    # Otsu's thresholding
    ret2, th2 = cv2.threshold(r, 128, 255, cv2.THRESH_TRIANGLE)

    th2 = cv2.erode(th2,np.ones((11, 11)))

    # ######################################################################
    # # Small spurious objects are easily removed by setting a minimum size for valid objects.
    cleaned_image = morphology.remove_small_objects(th2, 100,connectivity=8)
    ...
    # ######################################################################
    # # Small spurious objects are easily removed by setting a minimum size for valid objects.
    # cleaned_image = morphology.remove_small_objects(cleaned_image, 150)
    #
    # cv2.imshow("",th2)
    # cv2.waitKey(0)
...
def getGroundTruths(dataset):
    ...
    df.to_csv("Triangle_Segmentation_report.csv")
